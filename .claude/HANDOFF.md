...
## Blockers
None blocking. Three open items:

1. **INV-03 unverified.** `pre-commit` is not installed on this machine, so the gitleaks backstop has never actually run. A manual grep found nothing, but that is weaker. **Install `pre-commit` and run `pre-commit install`** — the repo ships `.pre-commit-config.yaml` and nothing is enforcing it.
2. **MAST FC1 category percentage is unresolved.** Three retrieval attempts gave 43.8 / 43.9 / 44.2 (mode-sum). The number is withheld from doctrine and the discrepancy is documented in `SOURCES.md#mast`. Resolve by reading Figure 1 of the paper directly if it ever matters.
3. **`guard-fanout.sh` is on probation.** Opt-in, not wired into `settings.json`. Tagged in `measurement.md` for a keep/cut decision at the next monthly review.
...
## Test state
- INV-01 ✅, INV-02 ✅, INV-04 ✅ — all with pasted evidence
- INV-03 ⚠ — tooling absent, see Blockers
