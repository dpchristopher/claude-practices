...
- **MAST verification changed two numbers.** Gating the claims on the primary PDF was worth it: system-design failures are **43.8%** (not the circulating 44.2%) and inter-agent misalignment **32.35%** (not ~37%). The paraphrased conclusion was also not the paper's operative sentence. Full 14-mode taxonomy and verbatim quotes are in `SOURCES.md#mast`. Gained category FC3 *Task Verification* (23.5%) entirely — of which incomplete or incorrect verification is 17.3% of all failures, a measured case for maker≠checker.
