...
1. Update `META_ARCHITECTURE.md` if any tool, data flow, or gap changed — dispatch `jerry-docs`,
   then confirm with `git diff --stat` that the files it claims to have edited actually changed
...
5. **Record decisions:** if the session chose between approaches, retired a rule, or declined an
   option, add it to `DECISIONS.md` with the reason, what was rejected, and what would reverse it.
   Skip if nothing was decided — do not manufacture entries.
6. **Feynman gate:** invoke `/feynman-explainer` — explain the session's changes in plain language. Stalls are comprehension gaps; resolve them. The resulting explanation is the HANDOFF body.
7. Write `.claude/HANDOFF.md` (replace, not append):
   ```
   ## Completed
   - [bullet]
   ...
   - [bullet]
   ```
8. Commit with session name in commit message: `"[session-name]: [what and why]"`
